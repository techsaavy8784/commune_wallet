    ...
    <div className={styles.page}>
      <>
        <Header
          ...
          type={'wallet'}
        />
        <div className={styles.pagecont} ref={dropDownRef}>
          {step1 ? (
            <div style={{ width: '100vw' }}>
              <div className={styles.communeinputLabel}>
                {i18nT('walletAddressBook.addRecp')}
              </div>
              <AddressInput
          ...
          {ensAddressObj?.address != null ? (
            <div className={styles.listscrollcont}>
              <div className={styles.listitemscont}>
                <div
                  onClick={() =>
                    replaceQuery('recipient', ensAddressObj?.address)
                  }
                ...
                >
                  <img
                    ...
                    src={
                      getSelectedAsset(selectedAsset)?.icon ||
                      getTokenInfo(selectedAsset)?.icon ||
                      getInfoBySymbol(selectedAccount.symbol).icon
                    }
                  />
                  <div className={styles.listinfo}>
                    <div className={styles.listtitle}>{ensAddressObj?.ens}</div>
                    <div className={styles.listsubtitle}>
                      {ensAddressObj?.address}
                    </div>
                  </div>

                  <img className={styles.listforward} src={ICON_FORWARD} />
                </div>
              </div>
            </div>
          ) : (
            <div>
                ...
                <div className={styles.listscrollcont}>
                  {myAccounts?.length == 0 ? (
                    <div className={styles.centerDiv}>
                      {i18nT('walletAddressBook.noPers')}
                    </div>
                  ) : (
                    <div className={styles.listitemscont}>
                      {filterAccount().map(
                        (account: keyable, index: number) => (
                          <div
                            key={index}
                            onClick={() =>
                              replaceQuery(
                                'recipient',
                                getTokenInfo(selectedAsset).type == 'DIP20'
                                  ? account?.meta?.principalId
                                  : account?.address
                              )
                            }
                            className={styles.listitem}
              ...
              {tab == 1 && (
                <div className={styles.listscrollcont}>
                  {recents?.length == 0 ? (
                    <div className={styles.centerDiv}>
                      {i18nT('walletAddressBook.noRecent')}
                    </div>
                  ) : (
                    <div className={styles.listitemscont}>
                      {recents?.map((recent: keyable, index: number) => (
                        <div
                          key={index}
                          onClick={() =>
                            replaceQuery('recipient', recent?.address)
                          }
                          className={styles.listitem}
                        >
                          <img
                          ...
                          <div className={styles.listinfo}>
                            ...
                            <div className={styles.listsubtitle}>
                              {i18nT('walletAddressBook.lastSent')}{' '}
                              {recent.lastSentAt &&
                                moment(recent?.lastSentAt).format(
                                  'Do MMMM YYYY'
                                )}
                            </div>
                          </div>
