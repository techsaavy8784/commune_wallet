...
const StakeEthConfirm = ({
  ...
  const sign = async () => {
    ...
    if (uniswap?.outputToken == 'ETH') {
      setIsBusy(false);
      try {
        const txn = await transferUniswap(
          uniswap,
  ...
  return (
    <div className={styles.page}>
      <Header
        type={'wallet'}
        text={
          uniswap?.outputToken == 'ETH'
            ? i18nT('stakeEthConfirm.unstake') + ' ETH'
            : i18nT('stakeEthConfirm.stake') + ' ETH'
        }
      >
      ...
      <div className={styles.ethWrapContainer}>
        <span style={{ textAlign: 'right' }} className={styles.ethWrapText}>
          {uniswap?.inputAmount}
        </span>
        <img
          src={uniswap?.outputToken == 'ETH' ? ICON_ROCKETPOOL : ICON_ETH}
          className={styles.logo}
        />
        <span className={styles.ethWrapText}>
          {uniswap?.outputToken == 'ETH' ? 'rETH' : 'ETH'}
        </span>
      </div>
      ...
      <div className={styles.internetCompWrapContainer}>
        <div className={styles.ethIconContainer}>
          <img
            src={uniswap?.outputToken != 'ETH' ? ICON_ROCKETPOOL : ICON_ETH}
        ...
        <div className={styles.ethTextContainer}>
          <span className={styles.ethereumText}>
            {uniswap?.outputToken == 'ETH' ? 'Ethereum' : 'Rocket Pool ETH'}
          </span>
          <span className={styles.ethVal}>
            {parseFloat(uniswap?.outputAmount).toFixed(5)}{' '}
            {uniswap?.outputToken == 'ETH' ? 'ETH' : 'rETH'}
          </span>
          <span className={styles.usdText}>
            $
            {uniswap?.outputToken == 'ETH'
              ? (((uniswap?.inputAmount * 1) / ratio) * usd).toFixed(2)
              : (uniswap?.inputAmount * usd).toFixed(2)}
          </span>
          {uniswap?.outputToken == 'ETH' ? (
            <span className={styles.conversionText}>
              1 rETH = {(1 / ratio).toFixed(5)} ETH
            </span>
          ) : (
            <span className={styles.conversionText}>
              1 ETH = {(1 / ratio).toFixed(5)} rETH
            </span>
          )}
        </div>
        <div className={styles.gasFeeContainer}>
          <div className={styles.leftSideContainer}>
            <span className={styles.gasFeeText}>
              {i18nT('stakeEthConfirm.estGas')}
            </span>
          </div>
          <div className={styles.rightSideContainer}>
            <span className={styles.communeText}>{gas?.toFixed(5)} ETH</span>
            <span className={styles.convertedVal}>
              ${uniswap?.estimatedGasUsedUSDTxt}
            </span>
          </div>
        </div>
        <div className={styles.totalContainer}>
          <span className={styles.totalText}>
            {i18nT('stakeEthConfirm.total')}
          </span>
          <div className={styles.rightSideTotalContainer}>
            <span className={styles.totalcommuneVal}>
              {((selectedAmount * 1) / ratio + gas + communeFees).toFixed(5)}{' '}
              ETH
            </span>
            <span className={styles.totalUSDVal}>
              $
              {((parseFloat(selectedAmount) + gas + communeFees) * usd).toFixed(
                2
              )}
            </span>
          </div>
      ...
      {isBusy ? (
        ...
        <section className={styles.footer}>
          ...
          <div className={styles.actions}>
            ...
            <ActionButton disabled={error != 'NO_ERROR'} onClick={() => sign()}>
              {uniswap?.outputToken == 'ETH'
                ? i18nT('stakeEthConfirm.unstake')
                : i18nT('stakeEthConfirm.stake')}
            </ActionButton>
          </div>
        </section>
      )}
