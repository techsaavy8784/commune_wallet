...
const WalletSendTokens = ({
  ...
  React.useEffect(() => {
    if (
      getSelectedAsset(selectedAsset)?.format == 'token' &&
  ...
  const transferForAll = async () => {
    ...
    try {
      ...
      setTxCompleteTxt(i18nT('walletSendTokens.payDone') || hash || '');
      setDone(true);
      setIsBusy(false);
    } catch (error) {
    ...
    if (isJsonString(secret)) {
      ...
      if (selectedAsset === selectedAccount?.symbol) {
        try {
          ...
          console.log(error);
          setTxError(
            i18nT('walletSendTokens.tryAgain') + JSON.stringify(error)
          );
          setLoadingSend(false);
          setIsBusy(false);
        }
        ...
        } else if (getSelectedAsset(selectedAsset)?.type == 'CommuneArt') {
          const callback = (path: string) => console.log(path);
          controller.assets
            .transferCommuneArt(
              secret,
        ...
        <div className={styles.pagecont}>
          ...
          {step1 ? (
            <div className={styles.innercontainer}>
              <div className={styles.communeinputLabel}>
                {i18nT('walletSendTokens.addRecp')}
              </div>
              <AddressInput
              ...
              <div className={styles.assetSelectionDivCont}>
                <div className={styles.communeinputLabel}>
                  {i18nT('walletSendTokens.selectedAsset')}
                </div>
              ...
              {selectedAsset === selectedAccount?.symbol ? (
                <div className={styles.confirmAmountCont}>
                  <img
                    ...
                    src={
                      getSymbol(selectedAccount?.symbol)?.icon ||
                      ICON_PLACEHOLDER
                    }
                  ...
                  <div>
                    <div className={styles.tokenText}>
                      {getTokenInfo(selectedAsset)?.name ||
                        getSelectedAsset(selectedAsset)?.name}
                    </div>
                    <div className={styles.tokenAmount}>
                      {selectedAmount.toFixed(5)}{' '}
                      {getTokenInfo(selectedAsset)?.symbol ||
                        getSelectedAsset(selectedAsset)?.symbol}
                    </div>
                    {!isNaN(getSelectedAsset(selectedAsset)?.usd) && (
                      <div className={styles.tokenValue}>
                        $
                        {(
                    ...
                    selectedAccount.symbol == 'ETH' ? (
                      ...
                      <div>
                        ...
                        <div className={styles.feeValue}>
                          $
                          {(
                            fees * getSelectedAsset(selectedAsset)?.usd
                          ).toFixed(3)}
                        </div>
                      </div>
                    )}
...
const AmountInput = ({
  ...
  const price =
    tokenInfo?.type == 'DIP20' ||
    tokenInfo?.type == 'ERC20' ||
    tokenInfo?.format == 'token'
      ? tokenInfo?.usd
      : currentUSDValue?.usd;
  ...
  const loadMaxAmount = useCallback((): void => {
    ...
    if (tokenInfo?.type == 'DIP20') {
      ...
      if (
        ...
        maxAmount = getMaxAmount_ETH(
          (
            currentBalance?.value /
            Math.pow(10, currentBalance?.currency?.decimals)
          ).toString(),
          feesArr[feesOptionSelected]
        );
      }
    }
  ...
  const changeAmount = (amount: string) => {
    ...
    if (isNaN(_amount)) {
      ...
      if (fees == 0) {
        ...
        setSelectedAmount(_amount);
        setError(
          i18nT('walletSendTokens.noZeroWith') +
            ` ${fees} ${selectedAccount?.symbol}`
        );
      }
