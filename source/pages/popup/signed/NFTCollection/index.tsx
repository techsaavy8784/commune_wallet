    ...
    const fetchNfts = async () => {
      ...
      if (nftCollObj.type != 'CommuneArt') {
        ...
        const listings = response.map((list: keyable) => {
          ...
          return {
            tokenId: getTokenIdentifier(collectionId, tokenIndex),
            id: tokenIndex,
    ...
    <div className={styles.page}>
      ...
      <div className={styles.mainContainer}>
          ...
          <div className={styles.statrow}>
            <div className={styles.statcol}>
              <div className={styles.key}>
                {i18nT('nftCollection.listings')}
              </div>
              <div className={styles.val}>
                {nftObj?.listings
                  ? millify(nftObj?.listings, {
                      precision: 2,
                      lowercase: true,
                    })
                  : listings.length || '-'}
            ...
            <div className={styles.statcol}>
              <div className={styles.key}>
                {i18nT('nftCollection.collectionSize')}
              </div>
              <div className={styles.val}>
                {nftObj?.tokens
                  ? millify(nftObj?.tokens, {
                      precision: 2,
                      lowercase: true,
                    })
                  : '-'}
        ...
        {loading
          ...
          : listings.map((nftObj: keyable) => (
              <div
                key={nftObj?.tokenId}
                className={styles.nftcardcont}
                onClick={() =>
                  history.push(
                    `/nft/buy/${nftObj?.tokenId}?price=${nftObj?.price}&accountId=${accountId}&type=${nftCollObj.type}`
                  )
                }
              >
                <NFTCard
                  ...
                  price={
                    (nftObj?.price / Math.pow(10, 8)) * currentUSDValue?.usd
                  }
                />
              </div>
            ))}
