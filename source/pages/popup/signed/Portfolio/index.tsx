        ...
        <div className={styles.cardcontinner}>
          <div className={styles.cardinfo}>
            ...
            <div className={styles.value}>
              <GroupBalance loading={loading} account={accounts[0]} />
            </div>
    ...
    <div className={styles.page}>
      ...
      >
        <div className={styles.headerIcons}>
          ...
          <div
            onClick={() =>
              context === true ? setContext(false) : setContext(true)
            }
          ...
          {context && (
            <div className={clsx(styles.headerContext)}>
              <Link className={styles.link} to={'/account/create'}>
                <div className={clsx(styles.headerContextLink)}>
                  Create new Porfolio
                </div>
              </Link>
              <Link className={styles.link} to={'/account/import'}>
      ...
      <div className={styles.cards}>
        {accountGroups?.sort(
          (a, b) => b[0].meta.createdAt - a[0].meta.createdAt
        ).map &&
          accountGroups?.map((accounts: any, index: number) => (
            <AccountsCard
              key={index}
              loading={loading}
              index={index}
...
const Balance = ({ account }: { account: keyable }) => {
  const currentBalance: keyable = useSelector(
    selectBalanceById(account.address)
  );
  return (
    <div>
...
const GroupBalance = ({
  ...
  const currentBalance: keyable = useSelector(
    selectGroupBalanceByAddress(account?.groupId)
  );
  return (
    <div>
      ${currentBalance?.balanceInUSD?.toFixed(3) || 0}
      {loading && (
        <span className={styles.totalSpinner}>
  ...
  return (
    <div className={styles.netlast}>
      ...
      <div className={styles.netstats}>
        ${currentBalance?.balanceInUSD?.toFixed(3)}
        {currentBalance?.usd_24h_change &&
          currentBalance?.balanceInUSD !== 0 && (
            <span
              className={
                currentBalance?.usd_24h_change > 0
                  ? styles.netstatspositive
                  : styles.netstatsnegative
              }
            >
              {currentBalance?.usd_24h_change?.toFixed(2)}%
            </span>
          )}
      </div>
    </div>
