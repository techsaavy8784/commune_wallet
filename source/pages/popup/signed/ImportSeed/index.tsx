...
const Page = () => {
  ...
  const _onNextStep = useCallback(() => {
    seed !== null &&
      controller.accounts.migrateExistingICP(seed).then((keypairGroup) => {
        if (keypairGroup?.balance?.balances[0]?.value > 0) {
          setAddresses({
            old: keypairGroup.keypair.address,
            new: keypairGroup.keypairNew.address,
          });
          setBalance(
            keypairGroup?.balance?.balances[0]?.value /
  ...
  const sendICPFromOldToNew = useCallback(async () => {
    if (seed !== null) {
      ...
      try {
        ...
        setTxError(
          'Please try again later or Skip! Error: ' + JSON.stringify(error)
        );
        setLoadingSend(false);
        setMigrateComplete(false);
      }
    ...
    (name: string, password: string): void => {
      if (name && password && seed) {
        ...
        const callback = (address: string) =>
          history.replace('/accounts?hightlight=' + address);
    ...
    switch (step) {
        ...
        return (
          <div>
            <div className={styles.communeinputCont}>
              ...
              <SeedAndPath onSeedChange={setSeed} onNextStep={_onNextStep} />
            </div>
          </div>
        );
      case '1a':
        return (
          <>
            <div className={styles.noticeCont}>
              <div className={styles.noticeTitle}>
                ...
                <div className={styles.noticeSubTitle}>
                  Internet Computer - ICP recommends you use new Secp256k1
                  address instead of old Ed25519 address. Your balance of{' '}
                  {balance} ICP shall be transferred to new address.
                </div>
              </div>
              <div>
                ...
                <div className={styles.notAddrCont}>
                  <img
                    src={getSymbol('ICP')?.icon}
                    className={styles.notAddrIcon}
                  ></img>
            ...
            {loadingSend ? (
              ...
              >
                <div onClick={() => setStep('2')} className={styles.secButton}>
                  {i18nT('importSeed.skip')}
                </div>
                <div
                  onClick={() => sendICPFromOldToNew()}
                  className={styles.primButton}
                >
                  {i18nT('importSeed.next')}
                </div>
              </div>
            )}
          </>
      ...
      case '2':
        return (
          <div>
            {migrateComplete && (
              <div className={styles.migrateAnnon}>
                <img
                  className={styles.migrateAnnonIcon}
                  src={ICON_GREEN_TICK}
                />
                {i18nT('importSeed.migrationComplete')}
                {'>'}{' '}
              </div>
            )}
      ...
      default:
        return <ClipLoader color={'#fffff'} size={15} />;
    }
  };
