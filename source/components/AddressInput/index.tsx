...
export default function AddressInput({
  ...
  const fetchData = async (
    ...
    symbol: string,
    tokenType: string,
    cb: (arg0: { address: string | null; ens: string }) => void
  ) => {
    ...
    <>
      <div className={styles.cont}>
        ...
        (isSearched == false
          ...
          : selectedRecp == ''
          ? false
          : ensAddressObj?.ens != selectedRecp) ? (
          <div className={styles.info}>
            <ClipLoader color={'#fffff'} size={15} />
          </div>
        ) : search ? (
          <img className={styles.info_search} src={ICON_SEARCH} />
        ) : valid ? (
          <img src={ICON_VALID_ADDRESS} className={styles.info_search} />
        ) : (
          <div className={styles.info}>
            <ToolTipInfo
              title={
                inputType == 'ICP'
                  ? tokenInfo?.addressType == 'principal'
        ...
        {search ? (
          <input
            ...
            className={clsx(
              styles.communeinput,
              tokenInfo?.addressType == 'principal' && styles.communeinput_princ
            )}
            placeholder={
              inputType == 'ICP'
                ? tokenInfo?.addressType == 'principal'
        ...
        {inputType == 'ETH' || inputType == 'MATIC' ? (
          <></>
        ) : inputType == 'BTC' ? (
          <div className={styles.type}>BTC</div>
        ) : inputType == 'ICP' && tokenInfo?.addressType == 'principal' ? (
          <div className={styles.type}>PRINC</div>
        ) : inputType == 'DOGE' ? (
          <></>
        ) : (
      ...
      {recpError !== '' && (
        <Warning isBelowInput isDanger className={styles.warningRecp}>
          {recpError}{' '}
          {recpError === PRINCIPAL_NOT_ACCEPTED && (
            <div
              onClick={() => togglePrincipal()}
              className={styles.communeLink}
            >
              Click here to change principal id to address
            </div>
          )}
        </Warning>
      )}
