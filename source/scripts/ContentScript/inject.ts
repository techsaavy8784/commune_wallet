...
window.commune = {
  ...
  on: (method, callback) => {
    ...
    window.commune.evtRegMap[id] = callback;

    window.addEventListener(id, ({detail}) => {
      const rCallback = window.commune.evtRegMap[id];
      if (rCallback) {
        rCallback(JSON.parse(detail));
      }
    })

    window.postMessage({ id, type: 'COMMUNE_EVENT_MESSAGE', data: { method } }, '*')
  }
}
`;

export { providerManager, CommuneProvider, ethereumProvider };
